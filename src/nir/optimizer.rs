...
fn folded_constant(op: &NirOp) -> Option<(TempId, NirValue)> {
    match op {
        ...
        NirOp::Cast { dest, src, from, to, .. } => {
            let bits = const_integer_bits(src)?;
            let bits = if let Some(integer) = from.kind.integer()
                && integer.signed && bits & (1u64 << (integer.bits - 1)) != 0
            { bits | !integer.mask() } else { bits };
            Some((*dest, value_for_type(bits, to)?))
        }
        NirOp::Binary {
            dest,
            ty,
...
fn eval_binary(op: NirBinaryOp, left: &NirValue, right: &NirValue, ty: &NirType) -> Option<u64> {
    ...
    match op {
        ...
        NirBinaryOp::Div | NirBinaryOp::Mod => {
            ...
            let result = crate::semantic::integer::divmod_bits(integer.bits, integer.signed, left, right).ok()?;
        ...
        NirBinaryOp::Lsh | NirBinaryOp::Rsh => Some(0),
    }
}
...
pub(super) fn binary_may_fault(op: &NirOp) -> bool {
    matches!(op, NirOp::Binary { op: NirBinaryOp::Div | NirBinaryOp::Mod, right, .. }
        if const_integer_bits(right).is_none_or(|value| value == 0))
}
