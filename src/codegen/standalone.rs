...
pub(crate) fn generate_semir_standalone_profile_at_origin(
    ...
    let mut compound_operations = application_projection.compound_operations;
    ...
    compound_operations.extend(projection.compound_operations.clone());
    ...
    compound_operations.extend(syslib.compound_operations.clone());
    ...
    if let Some(program_entry) = program_entry {
        let address = output
            ...
            .ok_or_else(|| {
                diagnostic(format!(
                    "classic standalone output lost application entry `{program_entry}`"
                ))
            })?;
...
fn runtime_image_projection(
    ...
    Ok(RuntimeProjection {
        ...
        compound_operations: projection.compound_operations,
