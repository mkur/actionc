    ...
    fn only_verified_algebraic_capabilities_are_open_in_public_profiles() {
        for target in [
            ...
            assert_eq!(SemanticOptions::modern().with_target(target).algebraic_types,
                AlgebraicTypeCapabilities { aggregate_values: true, variants: true, aggregate_calls: true, indirect_aggregate_calls: true, generic_types: true, ..AlgebraicTypeCapabilities::DISABLED });
        }
    }
