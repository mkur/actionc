...
fn runtime_bindings(
        ...
        .any(|declaration| runtime::requires_error(declaration.helper))
    {
        ...
        bindings.push(crate::codegen::CodegenRuntimeBinding {
            ...
            reason: if mir.runtime_helpers.iter().any(|decl| matches!(decl.helper, MirRuntimeHelper::Fault(_))) {
                "runtime fault (specific Error code identifies the reason)"
...
fn runtime_selection_reason(helper: MirRuntimeHelper) -> &'static str {
    match helper {
        ...
        MirRuntimeHelper::Fault(_) => "runtime fault requires a nonreturning Error handler",
    }
}
...
mod tests {
    ...
    #[test]
    fn source_generation_selects_local_backed_dual_pointer_word_copy() {
        let materialized = materialize_mir6502_source(
            "
            PROC Copy()
              CARD POINTER source,destination
              destination^=source^
            RETURN
            ",
        );
        ...
        assert!(
            ...
            "local-backed pointer copy should select the dual-pointer transfer:\n{}",
            format_program(&materialized)
        );
    }
