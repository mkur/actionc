...
fn estimated_op_cost(op: &MirOp) -> (u16, u16) {
    match op {
        ...
        MirOp::BinaryDirectIndexedByte { source, .. } => match source {
            crate::mir6502::ir::MirByteIndexedSource::Absolute { .. } => (3, 4),
            crate::mir6502::ir::MirByteIndexedSource::FixedIndirectY { .. } => (2, 5),
        },
        ...
        MirOp::OffsetPointerByIndirectByte { .. } => (15, 24),
        MirOp::Compare { width, .. } => width_cost(*width, (2, 2), (6, 8)),
        MirOp::CompareDirectIndexedBytes { .. } => (6, 8),
        MirOp::CompareIndirectBytes { .. } => (6, 12),
        MirOp::CompareIndirectWords { .. } => (9, 20),
