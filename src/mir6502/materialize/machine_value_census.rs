...
pub(super) fn fold_redundant_xy_reloads(
    ...
    for routine in &mut program.routines {
        let removals = {
            let memory_map = MirMachineMemoryMap::from_routine(routine, |mem| match mem {
...
mod tests {
    ...
    use crate::mir6502::ir::{
        MirBlock, MirBlockId, MirCond, MirEdge, MirEffects, MirFlagTest, MirFrame, MirGlobal,
        MirGlobalBacking, MirMem, MirRoutine, MirRoutineAbi, MirSpillId, MirTerminator, RoutineId,
    };
    ...
    fn fold_removes_redundant_x_reload_when_result_flags_are_dead() {
        ...
        fold_redundant_xy_reloads(&mut program, &layout, &mut stats);
