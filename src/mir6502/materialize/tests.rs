...
#[path = "compare_branch_copy_tests.rs"]
mod compare_branch_copy_tests;
...
#[test]
fn direct_unsigned_word_gt_branch_reverses_operands() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let mut blocks = direct_unsigned_word_compare_blocks(MirCompareOp::Gt);
...
#[test]
fn direct_unsigned_word_le_branch_reverses_operands_and_tests_set_carry() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let mut blocks = direct_unsigned_word_compare_blocks(MirCompareOp::Le);
    ...
    assert!(matches!(
        blocks[0].ops.first(),
        Some(MirOp::Compare {
            left: MirValue::PointerCell(MirMem::Local { id: LocalId(1), .. }),
            right: MirValue::PointerCell(MirMem::Local { id: LocalId(0), .. }),
            ..
        })
    ));
    assert!(matches!(
...
fn word_arithmetic_indirect_store_ops(destination_pointer: LocalId) -> Vec<MirOp> {
    vec![
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(300)),
        ...
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(301)),
            src: MirAddr::Direct(MirMem::Param {
                id: ParamId(0),
                offset: 0,
            }),
        ...
        MirOp::Binary {
            op: MirBinaryOp::Add,
            dst: MirDef::VTemp(MirTempId(302)),
            left: MirValue::Def(MirDef::VTemp(MirTempId(300))),
            right: MirValue::Def(MirDef::VTemp(MirTempId(301))),
            width: MirWidth::Word,
            carry_in: None,
            carry_out: MirCarryOut::Ignore,
        },
        MirOp::Store {
            ...
            src: MirValue::Def(MirDef::VTemp(MirTempId(302))),
            width: MirWidth::Word,
        },
    ]
}
...
#[test]
fn word_arithmetic_indirect_store_reads_both_lanes_before_writing() {
    let ops = word_arithmetic_indirect_store_ops(LocalId(30));
    let mut replacement = Vec::new();

    assert_eq!(
        select_word_arithmetic_indirect_store_consumer(&ops, 0, &mut replacement),
        ops.len()
    );
    assert!(matches!(
...
#[test]
fn word_arithmetic_indirect_store_requires_one_shared_pointer() {
    let ops = word_arithmetic_indirect_store_ops(LocalId(31));
    let mut replacement = Vec::new();
...
fn word_arithmetic_pointer_store_ops(pointer_use_home: LocalId) -> Vec<MirOp> {
    vec![
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(310)),
            src: MirAddr::Direct(MirMem::Local {
                id: LocalId(32),
                offset: 0,
            }),
        ...
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(311)),
            src: MirAddr::PointerCell {
                ptr: MirMem::Local {
                    id: LocalId(32),
                    offset: 0,
        ...
        MirOp::Binary {
            op: MirBinaryOp::Add,
            dst: MirDef::VTemp(MirTempId(312)),
            left: MirValue::Def(MirDef::VTemp(MirTempId(310))),
            right: MirValue::Def(MirDef::VTemp(MirTempId(311))),
            width: MirWidth::Word,
            carry_in: None,
            carry_out: MirCarryOut::Ignore,
        },
        MirOp::Store {
            ...
            src: MirValue::Def(MirDef::VTemp(MirTempId(312))),
            width: MirWidth::Word,
        },
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(313)),
            src: MirAddr::Direct(MirMem::Param {
                id: ParamId(0),
                offset: 0,
            }),
        ...
        MirOp::Store {
            ...
            src: MirValue::Def(MirDef::VTemp(MirTempId(313))),
            width: MirWidth::Word,
        },
    ]
}
...
#[test]
fn word_arithmetic_pointer_store_keeps_home_and_prepares_consumer() {
    let ops = word_arithmetic_pointer_store_ops(LocalId(33));
    let mut replacement = Vec::new();

    assert_eq!(
        select_word_arithmetic_pointer_store_consumer(&ops, 0, &mut replacement),
        ops.len()
    );
    ...
    let first_indirect_store = replacement
        .iter()
        .position(|op| matches!(op, MirOp::StoreIndirect { .. }))
        .expect("pointer payload store");
    assert_eq!(
        replacement[..first_indirect_store]
            .iter()
            .filter(|op| {
                matches!(
                    op,
                    MirOp::Store {
                        dst: MirAddr::Direct(MirMem::FixedZeroPage(MirFixedZpSlot(
                            POINTER_INDEX_SCRATCH_LO | POINTER_INDEX_SCRATCH_HI
                        ))),
                        ..
                    }
                )
            ...
            .count(),
        4,
        "both arithmetic-result and payload words are staged before pointer writes"
    );
    assert!(matches!(
        &replacement[first_indirect_store..],
        [
...
#[test]
fn word_arithmetic_pointer_store_requires_the_written_home_as_pointer() {
    let ops = word_arithmetic_pointer_store_ops(LocalId(34));
    let mut replacement = Vec::new();
...
fn word_carry_chain_store_reuses_updated_pointer_for_followup_deref() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let base = MirDef::VTemp(MirTempId(189));
    let addend = MirDef::VTemp(MirTempId(190));
    let sum = MirDef::VTemp(MirTempId(191));
    let result = MirDef::VTemp(MirTempId(192));
    let loaded = MirDef::VTemp(MirTempId(193));
    let pointer = MirMem::Param {
        id: ParamId(0),
        offset: 0,
    };
    ...
    let ops = vec![
        MirOp::Load {
            dst: base.clone(),
            src: MirAddr::Direct(pointer.clone()),
            width: MirWidth::Word,
        },
        MirOp::Load {
            dst: addend.clone(),
...
#[test]
fn local_indirect_word_copy_uses_overlap_safe_dual_pointer_op() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let value = MirDef::VTemp(MirTempId(0));
    ...
    let destination = pointer_value_from_mem(&MirMem::Local {
        id: LocalId(1),
        offset: 0,
    });
    let ops = vec![
...
#[test]
fn analyzed_ssa_lite_removes_accumulator_reload_from_branch_edge() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let source = MirMem::Local {
        id: LocalId(0),
    ...
    let compare_a = |value| MirOp::Compare {
        dst: MirCondDest::Flags,
        op: MirCompareOp::Eq,
        left: MirValue::Def(MirDef::Reg(MirReg::A)),
    ...
    assert_eq!(result.estimated_bytes_saved, 3);
    assert_eq!(routine.blocks[1].ops, vec![compare_a(2)]);
}

#[test]
fn analyzed_ssa_lite_merges_identical_accumulator_values_at_join() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let source = MirMem::Local {
        id: LocalId(0),
    ...
    let mut routine = ssa_lite_edge_test_routine(vec![
        ...
        MirBlock {
            ...
            label: "join".to_string(),
            params: Vec::new(),
            ops: vec![load_source(), compare.clone()],
            terminator: MirTerminator::Return,
        },
    ]);
    ...
    assert_eq!(routine.blocks[3].ops, vec![compare]);
}

#[test]
fn analyzed_ssa_lite_rejects_differing_accumulator_values_at_join() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let source = MirMem::Local {
        id: LocalId(0),
...
fn analyzed_ssa_lite_keeps_edge_reload_after_incoming_flags_are_overwritten() {
    ...
    assert_eq!(result.blocked_by_reason.get("flags-live"), Some(&1));
    assert_eq!(routine.blocks[1].ops, vec![load_source()]);
}
...
fn pointer_byte_offset_mixed_call_ops(interleave_second_arg: bool) -> Vec<MirOp> {
    ...
    if interleave_second_arg {
        ops.push(MirOp::Load {
            dst: MirDef::VTemp(MirTempId(4)),
            src: MirAddr::Direct(p1.clone()),
            width: MirWidth::Word,
        });
    }
    ops.extend([
    ...
    ops.extend([
        ...
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(6)),
            src: MirAddr::Direct(p1),
            width: MirWidth::Word,
        },
        MirOp::Load {
            dst: MirDef::VTemp(MirTempId(7)),
            src: MirAddr::Deref {
                ptr: MirValue::Def(MirDef::VTemp(MirTempId(6))),
                offset: 0,
            },
            width: MirWidth::Byte,
        },
...
#[test]
fn call_arg_expr_stages_pointer_byte_offset_chain_in_canonical_action_homes() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let ops = pointer_byte_offset_mixed_call_ops(false);
    let mut helpers = Vec::new();
    let mut out = Vec::new();
    ...
    assert_eq!(result.consumed, ops.len());
    assert!(helpers.is_empty());
    assert!((0..=7).all(|temp| out.iter().all(|op| !op_uses_temp(op, MirTempId(temp)))));
    for offset in 0..=4 {
        assert!(out.iter().any(|op| matches!(
            op,
            MirOp::Store {
    ...
    assert!(matches!(
        out.last(),
        Some(MirOp::Call { args, abi, .. })
            if args.iter().enumerate().all(|(offset, arg)| {
                arg.value == MirValue::PointerCell(return_slot_mem(offset as u16))
            }) && abi.params == vec![
                MirArgHome::Reg(MirReg::A),
                MirArgHome::Reg(MirReg::X),
                MirArgHome::Reg(MirReg::Y),
...
#[test]
fn call_arg_expr_keeps_interleaved_pointer_byte_offset_argument_producers() {
    let program = empty_test_program();
    let layout = MaterializeLayout::new(&program, 0x3000);
    let ops = pointer_byte_offset_mixed_call_ops(true);
    let mut helpers = Vec::new();
    let mut out = Vec::new();
