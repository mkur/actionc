...
pub fn prepare_image(mut inputs: Vec<SourceInput>) -> PreparedImage {
    ...
    for source in &sources {
        append_field(&mut manifest, source.input.kind.as_bytes());
        append_field(&mut manifest, source.input.canonical_key.as_bytes());
        append_field(&mut manifest, source.input.virtual_path.as_bytes());
        append_field(&mut manifest, source.sha256.as_bytes());
    }
    PreparedImage {
