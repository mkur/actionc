...
fn incremental_indexes_preserve_rebinding_in_rhs_calls_and_zero_trip_loops() {
    for element in ["CARD", "LONGCARD"] {
        let source = common::Source::new(
         ...
         RETURN\n"
                .replace("CARD ARRAY", &format!("{element} ARRAY"))
                .replace("CARD FUNC", &format!("{element} FUNC")),
        );
...
#[test]
fn guarded_long_indexes_preserve_negative_coordinates_large_offsets_and_odd_rebasing() {
    let source = common::Source::new(
        "LONGCARD ARRAY storage(80002),grid(4,20000)
INT start
BYTE bias
PROC Main() INT row
  grid=@storage(40000)+bias
  FOR row=start TO 1 DO grid(row,0)=LONGCARD(row+5)*LONGCARD($10000001) OD
RETURN
",
    );
    for optimize in [false, true] {
        ...
        for start in [-2i16, 0, 1, 2] {
            for bias in [0u32, 1] {
                ...
                vm.write_scalar(image.symbol("start").unwrap(), start as u16 as u32)
                    .unwrap();
                vm.write_scalar(image.symbol("bias").unwrap(), bias)
                    .unwrap();
                vm.run(100_000).assert_completed();
                let mut expected = vec![0u8; 4 * 80002];
                for row in start..=1 {
                    let at = (160000 + i32::from(row) * 80000) as usize + bias as usize;
                    expected[at..at + 4]
                        .copy_from_slice(&((row + 5) as u32 * 0x10000001).to_be_bytes());
                }
                let address = image.symbol("storage").unwrap().address().unwrap();
                assert_eq!(
                    vm.cpu.mem.bytes(address, expected.len()).unwrap(),
                    expected,
                    "{optimize}/{start}/{bias}"
                );
            }
