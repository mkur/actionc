...
fn final_pointer_arguments_keep_complete_areas_results_and_reserved_homes_unread() {
    ...
    for local in [false, true] {
        let source = if local {
            source.replace("RETURN(Echo(3,p,LONGCARD($89abcdef),p,$4321))", "BYTE POINTER local local=p IF p=0 THEN local=p FI RETURN(Echo(3,local,LONGCARD($89abcdef),local,$4321))")
        } else {
            source.to_string()
        };
        ...
        for optimize in [false, true] {
            for fallback in [false, true] {
                ...
                assert_eq!(omitted.len(),r.blocks.iter().flat_map(|b|&b.ops).filter(|op|matches!(op,Mir65816Op::Load{width,address,..} if width.get()==3 && (if local {matches!(address.base,Mir65816AddressBase::AutomaticFrame(_))}else{matches!(address.base,Mir65816AddressBase::Parameter(_))}))).count());
                    ...
                    for value in [0u32, 0x12fffe, 0xffffff] {
                        for mask in [0, 4] {
                            ...
                            for (home, bits) in plan.arguments.iter().zip([
                                ...
                                let Mir65816AbiHome::StackArgument { offset, size, .. } = home
                                else {
                                    unreachable!()
                                };
                                expected
                                    [offset.get() as usize..(offset.get() + size.get()) as usize]
                                    .copy_from_slice(
                                        &u32::to_le_bytes(bits)[..size.get() as usize],
                                    );
                            }
