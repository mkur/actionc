...
fn incremental_argument_pushes_survive_irq_and_nmi_at_each_instruction() {
    for &(ty, width) in &[("CARD", 2u8), ("ADDRESS", 3), ("LONGCARD", 4)] {
        for immediate in [false, true] {
            ...
            for local in if width == 3 && !immediate {
                vec![false, true]
            } else {
                vec![false]
            } {
                let source = if local {
                    source.replace("RETURN(Echo(BYTE(value),value,BYTE(value)))", &format!("{ty} local local=value IF value=0 THEN local=value FI RETURN(Echo(BYTE(local),local,BYTE(local)))"))
                } else {
                    source.clone()
                };
                for optimize in [false, true] {
                    for domain in 0..2 {
                            ...
                            for mask in [0, 4] {
                                ...
                                for nmi in [false, true] {
                                    ...
                                    for _ in 0..100_000 {
                                        ...
                                        ack |= h.bus.writes[writes..].iter().any(|&(a, _)| {
                                            a == if nmi { NMI_ACK } else { IRQ_ACK }
                                        });
                        ...
                        assert_eq!(
                            ...
                            (if immediate { 0x1234u32 } else { value })
                                .wrapping_add(2 * (value & 255))
                                & (u32::MAX >> (8 * (4 - width)))
                        );
                    }
                }
