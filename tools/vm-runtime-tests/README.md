...
is checked by `cargo test --locked --test unknown_pleasures`. The test
...
also checks the integer data checksum, all 300 samples of 50 evenly spaced traces
against VBXE's quarter-scanline data, screen bounds, load ranges, palette, and
keypress exit to text mode. These checks need no external CSV or network access.
