...
fn state(vm: &CompilerVm, active: u16, tags: u16, location: u16) -> Vec<u16> {
    ...
    vec![
        ram.read_word(active),
        ram.read(location + 10).into(),
        ram.read(location + 11).into(),
        u16::from(ram.read(ram.read_word(tags) + 11)) * 0x7F,
        ram.read_word(active + 2),
        ram.read(active + 4).into(),
        ram.read(location).into(),
        ram.read(location + 1).into(),
    ]
}
...
pub(super) fn check(compiled: &CompiledProgram, debug: bool) {
    ...
    for (dostst, directory, ramdisk, other_drive) in [(0xA9, 0x07B8, 4, 4), (0, 0x07BB, 0xFF, 2)] {
        ...
        assert_eq!(
            state(&vm, active, tags, location)[3],
            0x7F,
            "tags={:?}",
            (0..14)
                .map(|o| vm.bus().ram().read(right_tags + o))
                .collect::<Vec<_>>()
        );
        ...
        assert_eq!(
            state(&vm, active, tags, location),
            [1, 0, u16::from(other_drive), 0, 0, 0, 0x69, 1]
        );
        ...
        assert_eq!(
            vm.bus().ram().read_word(vm.bus().ram().read_word(tags) + 6),
            0
        );
        assert_eq!(vm.bus().ram().read_word(global("lefttags") + 6), 0);
        let ram = vm.bus_mut().ram_mut();
        ram.map(active, &[17, 0, 7, 0, 7]).unwrap();
        ...
        assert_eq!(
            state(&vm, active, tags, location),
            [17, 2, 5, 0, 7, 7, 0x43, 0x65]
        );
        vm = call(vm, &mut hooks, routine("SwapWin"), 0, 0);
        assert_eq!(
            state(&vm, active, tags, location),
            [24, 4, 3, 0x7F, 19, 15, 0x21, 0x43]
        );
