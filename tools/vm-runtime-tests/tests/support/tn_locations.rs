...
impl VmRunHooks for NoIo {
    type Error = String;
    fn before_step(&mut self, _: &mut CompilerVm) -> Result<(), String> {
        Ok(())
    }
}
...
pub fn check(compiled: &CompiledProgram) {
    ...
    vm.bus_mut()
        .ram_mut()
        .map(location - 1, &[0xA5; 51])
        .unwrap();
    let mut root_args = words(&[location]);
    root_args.push(2);
    ...
    for (level, stem) in ["ABCDEFGH", "IJKLMNOP", "QRSTUVWX", "YZ012345"]
        .iter()
        .enumerate()
    {
        ...
        vm.bus_mut()
            .ram_mut()
            .map(name + 1, filename.as_bytes())
            .unwrap();
        vm = machine::call(
            vm,
            &mut hooks,
            routine("MyDosPush"),
            &words(&[location, name, 0x169 + level as u16, 0x16A + level as u16]),
        );
        ...
        assert_eq!(
            vm.bus().ram().read(slot + 9),
            0xA5,
            "dot must not overwrite next name or guard"
        );
        let mut path_args = words(&[location]);
        path_args.push(level as u8 + 1);
    ...
    vm = machine::call(
        ...
        &mut hooks,
        routine("MyDosPush"),
        &words(&[location, name, 0x7777, 0x8888]),
    );
    ...
    for guard in [location - 1, location + 49] {
        assert_eq!(vm.bus().ram().read(guard), 0xA5);
    }
}
