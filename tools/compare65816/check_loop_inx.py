#!/usr/bin/env python3
"""Check frozen loop INX images, all counters, address maps and external controls."""
import argparse
import copy
import hashlib
import json
...
from loop_inx import transform, measurement, digest
...
def check(before_dir, after_dir, frozen):
        ...
        for projection in f['projections']:
            ...
            if t:
                traffic.append(dict(case=key[0], mode=key[1], vector=projection['vector'],
                    ...
                    x_increment_updates=e['x_increment_updates']))
    ...
    return dict(records=len(after), complete_action_images=len(forecasts),
        ...
        copy_and_prior_a_frame_parameter_forwarding_preserved=True, x_input_loads_replaced_by_increment_updates=True, guard_algorithm_and_abi_preserved=True,
...
def negative_controls(before_dir, after_dir, frozen):
    ...
    rejected = []
    with tempfile.TemporaryDirectory(prefix='loop-inx-counter-controls-') as directory:
        directory = Path(directory)
        (directory/'manifest.json').write_bytes((after_dir/'manifest.json').read_bytes())
        for field in ('cycles', 'dp_reads', 'dp_writes', 'x_forwarded_loads', 'x_increment_updates',
