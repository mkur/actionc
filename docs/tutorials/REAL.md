...
PROC Main()
  ...
  SYS.PrintE("Total:")
  SYS.PrintRE(@total) ;  pass the address of total
...
The sample closes IOCB 0 and explicitly reopens the editor device, so output
works when the XEX is booted directly in an emulator without `E:` already open.
