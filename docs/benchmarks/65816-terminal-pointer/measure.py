...
Usage: python3 -B docs/benchmarks/65816-terminal-pointer/measure.py BEFORE AFTER SLICE
...
def measure(before, after, slice_name):
    ...
    for x, y, m, n in zip(a['routines'], b['routines'], inventories[0]['routines'], inventories[1]['routines']):
        ...
        labels = n['labels'] if isinstance(n['labels'], dict) else dict(n['labels'])
        for jump in n['jumps']:
            target = labels.get(jump['target'], labels.get(str(jump['target'])))
    ...
    summary=dict(before=before, after=after, compiler_code_before=sum(r['size'] for r in a['routines']),
                 ...
                 cumulative_saved=331914-code,
                 newly_omitted_pointer_captures=sum(1 for row in spans if row['kind'] in ['Load/3/Parameter','Load/3/AutomaticFrame'] and row['before']>0 and row['after']==0),
