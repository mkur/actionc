...
NIR `CopyBytes` lowers to a pre-home MIR6502 `CopyBytes` operation carrying the
two lowered addresses, verified extent, and volatile flags. This retains
aggregate identity until target-copy selection instead of requiring later
passes to reconstruct it from unrelated scalar operations. The operation is
rejected after materialization and can never reach emission unresolved.

Target selection keeps direct endpoints as constant-offset byte accesses. For
`ComputedIndex`, `PointerIndex`, `PointerCell`, and `Deref` endpoints whose byte
range fits indirect Y addressing, it prepares one fixed pointer for the source
phase and reuses it for every source lane. After all source lanes have been
staged, it may prepare the same fixed pointer pair for the destination phase
and reuse it for every store. A scaled endpoint therefore has one base
materialization and one address advance, not one per copied byte.

Both the selected strategy and the correctness-first fallback load every
source lane into a distinct generated byte temporary before emitting any
destination store. Reusing already-normalized address and index values
preserves single evaluation, while staging the complete value makes
self-copies and arbitrary overlap safe. Volatile source and destination phases
retain explicit barriers around their respective memory operations. Address
forms or indirect offsets that cannot be represented exactly use the scalar
fallback.
