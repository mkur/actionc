...
The correctness-first fallback expands the operation into ordinary byte
`Load` and `Store` operations. MIR6502 first loads every source lane into a
distinct generated byte temporary and only then emits the destination stores.
Reusing the already-normalized address and index values preserves single
evaluation, while staging the complete value makes self-copies and arbitrary
overlap safe. Volatile source and destination phases retain explicit barriers
around their respective memory operations.
