...
The current listing is close to MADS syntax, but it is a presentation artifact:

```text
3000  4C 03 30  JMP $3003
```

Its address and byte columns are not assembler input, it has no `ORG`, and it
does not emit the load-file `RUNAD` segment. Simply stripping columns is also
unsafe because MADS may shorten an absolute operand in page zero unless the
original addressing width is made explicit.
...
MADS reproduced the complete 41-byte load file exactly. The implementation
should turn that probe into a maintained formatter and regression workflow.

There will be one listing syntax, not separate human and MADS formats. Address
...
The generated file should call itself a fixed-origin MADS assembly export in
its header comment. It should not claim to be original source or relocatable
compiler IR.
...
```asm
; Generated by actionc. Fixed-origin MADS assembly export.
...
- `src/compiler/artifacts.rs` consumes that final `CodegenOutput` and formats
  both listing variants with one MADS-compatible renderer.
