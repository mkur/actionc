...
MIR6502 receives the normalized copy operation and stages each source byte in a
generated byte temporary before emitting any destination store. This is an
overlap-safe correctness-first strategy; later target optimization may select a
smaller directional copy after proving the relevant address facts. The
existing `SYS.MoveBlock` routine is forward-copying, so it is not the semantic
definition of record assignment when overlap is possible.
