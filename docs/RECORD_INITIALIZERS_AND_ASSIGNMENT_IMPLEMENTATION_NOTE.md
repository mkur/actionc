...
correctness-first fallback stages each source byte in a generated byte
temporary before emitting any destination store. This is overlap-safe; later
target optimization may share prepared addresses or select a smaller
directional copy after proving the relevant address facts. The existing
`SYS.MoveBlock` routine is forward-copying, so it is not the semantic definition
of record assignment when overlap is possible.
