...
- Slices 2–5: pending. The rename audit also found that the native branch
  recognizes SYS-qualified type declarations but not SYS-qualified scalar
  casts. Integration must route these through canonical resolved cast facts,
  alongside the existing enum cast facts, with qualified-cast regression tests.
