//! Shared read-only typed movement export. Historical mode retains its schema.
#[path = "scalar_dp_facts.rs"]
mod scalar_dp_facts;
use actionc::{
...
fn export_mode(scalar: bool, registers: bool) {
        ...
        .filter(|a| a["compiler"] == "actionc")
    {
        ...
        for m in &c.machine.routines {
            ...
            let mut fact = json!({"id":r.id.0,"name":r.name,"address":placed.address,"size":placed.size,
        ...
        assert_eq!(
            routines.len(),
            if scalar {
                c.machine.routines.len()
            } else {
                a["routines"].as_array().unwrap().len()
            }
        );
