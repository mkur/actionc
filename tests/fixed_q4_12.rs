...
#[test]
fn fixed_q4_12_imports_and_all_functions_compile_in_all_modes_and_runtimes() {
    for (import, alias) in [("USE MATH.Q4_12", "Q4_12"), ("USE MATH.Q4_12 AS Q", "Q")] {
        let source = Source::new(&format!(
            "MODULE TEST {import}\nINT input=$6E0,result=$601 LONGCARD square=$605\nPROC Main()\n\
             result={alias}.FromInt(input) result={alias}.Trunc(result)\n\
             result={alias}.Mul(input,result) result={alias}.Div(input,result)\n\
             result={alias}.FromRatio(input,result) result={alias}.MulFloor(input,result) square={alias}.SqrWide(input)\n\
...
#[test]
fn fixed_q4_12_exports_typed_constants_without_loading_real() {
    let source = Source::new("MODULE TEST USE MATH.Q4_12 AS Q PROC Main() RETURN ENDMODULE");
    ...
    let origins: Vec<_> = loaded
        ...
        .collect();
    assert!(
        origins.iter().any(|s| s == "<embedded:MATH.Q4_12>"),
        "{origins:?}"
    );
    assert!(
        !origins
    ...
    for (name, bits) in [
        ...
        let qualified = format!("MATH.Q4_12.{name}");
