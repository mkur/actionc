...
#[test]
#[ignore = "executes standalone library groups with actionc-vm; use cargo test --test compatibility standalone_library_runtime_check -- --ignored"]
fn standalone_library_runtime_check() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let harness = repo_root.join("tools").join("vm-runtime-tests");

    let output = Command::new("cargo")
        .args(["test", "--locked", "selectively_linked_"])
        .current_dir(&harness)
        .output()
        .unwrap_or_else(|err| panic!("run standalone VM tests in {}: {err}", harness.display()));

    if !output.status.success() {
        panic!(
            "standalone VM tests failed with status {}\nstdout:\n{}\nstderr:\n{}",
            output.status,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
...
#[test]
#[ignore = "executes resident string/memory contracts with actionc-vm; use cargo test --test compatibility resident_memory_strings_runtime_check -- --ignored"]
fn resident_memory_strings_runtime_check() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let harness = repo_root.join("tools").join("vm-runtime-tests");

    let output = Command::new("cargo")
        .args(["test", "--locked", "resident_memory_and_string_"])
        .current_dir(&harness)
        .output()
        .unwrap_or_else(|err| panic!("run string/memory VM tests in {}: {err}", harness.display()));

    if !output.status.success() {
        panic!(
            "string/memory VM tests failed with status {}\nstdout:\n{}\nstderr:\n{}",
            output.status,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
...
#[test]
#[ignore = "executes resident hardware-helper contracts with actionc-vm; use cargo test --test compatibility resident_hardware_helpers_runtime_check -- --ignored"]
fn resident_hardware_helpers_runtime_check() {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let harness = repo_root.join("tools").join("vm-runtime-tests");

    let output = Command::new("cargo")
        .args(["test", "--locked", "resident_hardware_helpers_"])
        .current_dir(&harness)
        .output()
        .unwrap_or_else(|err| {
            panic!(
                "run hardware-helper VM tests in {}: {err}",
                harness.display()
            )
        });
    ...
    if !output.status.success() {
        panic!(
            "hardware-helper VM tests failed with status {}\nstdout:\n{}\nstderr:\n{}",
            output.status,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
        );
...
fn direct_byte_array_index_runtime_check() {
    ...
    let script = repo_root
        .join("fixtures")
        .join("runtime")
        .join("run-direct-byte-array-indexes-vm.sh");
