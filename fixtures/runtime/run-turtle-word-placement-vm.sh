...
out_dir="$(mktemp -d "${TMPDIR:-/tmp}/actionc-turtle-word-placement.XXXXXX")"
cleanup() {
  rm -rf "$out_dir"
}
trap cleanup EXIT
...
for backend in classic mir6502; do
  ...
  echo "==> TURTLE word placement: compile modern/$backend"
  (
    cd "$repo_root"
  ...
  echo "==> TURTLE word placement: execute modern/$backend"
